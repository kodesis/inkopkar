...
<script>
    let jquery_datatable = $("#table_artikel").DataTable({
        ...
        ajax: {
            url: "<?php echo site_url('Gallery_Management/ajax_list') ?> ",
            type: "POST",
            data: function(data) {}
        },
        columnDefs: [{
        ...
        if (!ttlthumbnailValue) {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var url;
                    var formData;
                    url = "<?php echo site_url('Gallery_Management/save') ?>";
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (!data.status) swal.fire('Gagal menyimpan data', 'error :' + data.Pesan);
                                ...
                                // location.reload();
                                setTimeout(function() {
                                    console.log('Redirecting to Gallery_Management...');
                                    location.href = '<?= base_url('Gallery_Management') ?>';
    ...
    function update_artikel() {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                var url;
                var formData;
                url = "<?php echo site_url('Gallery_Management/proses_update') ?>";
    ...
    function onDelete(id) {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "<?php echo site_url('Gallery_Management/delete') ?>",
                    type: "POST",
                    data: {
                        id_delete: id
                    },
    ...
    function uploadImage(file, editor, welEditable) {
        ...
        $.ajax({
            url: "<?php echo site_url('Gallery_Management/upload_summernote') ?>",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
