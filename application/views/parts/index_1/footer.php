<!-- footer-area -->
<footer>
    <div class="footer__area-two">
        <div class="footer__newsletter-two">
            <div class="container">
                <div class="footer__newsletter-inner">
                    <h2 class="title">Terima Kasih Telah Mengunjungi INKOPKAR</h2>
                    <!-- <form action="#">
                        <input type="text" placeholder="e-mail Type . . .">
                        <button type="submit" class="btn">Subscribe</button>
                    </form> -->
                    <div class="footer__social-two">
                        <ul class="list-wrap">
                            <!-- <li><a href="javascript:void(0)"><i class="fab fa-facebook-f"></i></a></li>
                            <li><a href="javascript:void(0)"><i class="fab fa-twitter"></i></a></li> -->
                            <li><a href="https://www.instagram.com/inkopkar_id" target="_blank"><i class="fab fa-instagram"></i></a></li>
                            <li><a href="https://www.tiktok.com/@inkopkar_id" target="_blank"><i class="fab fa-tiktok"></i></a></li>
                            <!-- <li><a href="javascript:void(0)"><i class="fab fa-youtube"></i></a></li> -->
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <div class="footer__top-two">
            <div class="container">
                <div class="row">
                    <div class="col-xl-7 col-lg-5 col-md-6">
                        <div class="footer-widget">
                            <div class="footer__content-two">
                                <div class="fw-logo mb-25">
                                    <a href="index.html"><img src="<?= base_url() ?>/assets/img/logo/logo.gif" alt=""></a>
                                </div>
                                <p>Bersama INKOPKAR, Membangun Masa Depan Lebih Baik!</p>
                                <div class="footer-info-list footer-info-two">
                                    <ul class="list-wrap">
                                        <li>
                                            <div class="icon">
                                                <i class="flaticon-phone-call"></i>
                                            </div>
                                            <div class="content">
                                                <a href="tel:0218290289">+62 21 8290289 - 8353631</a>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="icon">
                                                <i class="flaticon-envelope"></i>
                                            </div>
                                            <div class="content">
                                                <a href="mailto:admin@inkopkar.coop">admin@inkopkar.coop</a>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="icon">
                                                <i class="flaticon-pin"></i>
                                            </div>
                                            <div class="content">
                                                <p>Jl. Tebet Barat Dalam Raya No.15 <br> Jakarta Selatan 12810</p>
                                            </div>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-5 col-lg-3 col-sm-6">
                        <div class="footer-widget">
                            <h4 class="fw-title">Halaman Kami</h4>
                            <div class="footer-link-list">
                                <ul class="list-wrap">
                                    <li><a href="<?= base_url() ?>">Beranda</a></li>
                                    <li><a href="<?= base_url('about') ?>">Tentang Kami</a></li>
                                    <li><a href="<?= base_url('service') ?>">Layanan Kami</a></li>
                                    <li><a href="<?= base_url('blog') ?>">Blog</a></li>
                                    <li><a href="<?= base_url('contact') ?>">Kontak Kami</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <!-- <div class="col-xl-3 col-lg-4 col-sm-6">
                        <div class="footer-widget">
                            <h4 class="fw-title">Top Links</h4>
                            <div class="footer-link-list">
                                <ul class="list-wrap">
                                    <li><a href="about.html">How it’s Work</a></li>
                                    <li><a href="contact.html">Partners</a></li>
                                    <li><a href="contact.html">Testimonials</a></li>
                                    <li><a href="contact.html">Case Studies</a></li>
                                    <li><a href="contact.html">Pricing</a></li>
                                </ul>
                            </div>
                        </div>
                    </div> -->
                    <!-- <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="footer-widget">
                            <h4 class="fw-title">Instagram Posts</h4>
                            <div class="footer-instagram">
                                <ul class="list-wrap">
                                    <li><a href="javascript:void(0)"><img src="<?= base_url() ?>/assets/img/images/footer_insta01.jpg" alt=""></a></li>
                                    <li><a href="javascript:void(0)"><img src="<?= base_url() ?>/assets/img/images/footer_insta02.jpg" alt=""></a></li>
                                    <li><a href="javascript:void(0)"><img src="<?= base_url() ?>/assets/img/images/footer_insta03.jpg" alt=""></a></li>
                                    <li><a href="javascript:void(0)"><img src="<?= base_url() ?>/assets/img/images/footer_insta04.jpg" alt=""></a></li>
                                    <li><a href="javascript:void(0)"><img src="<?= base_url() ?>/assets/img/images/footer_insta05.jpg" alt=""></a></li>
                                    <li><a href="javascript:void(0)"><img src="<?= base_url() ?>/assets/img/images/footer_insta06.jpg" alt=""></a></li>
                                </ul>
                            </div>
                        </div>
                    </div> -->
                </div>
            </div>
        </div>
        <div class="footer__bottom-two">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="copyright-text-two">
                            <!-- <p>Copyright © <a href="index.html">Apexa</a> | All Right Reserved</p> -->
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
<!-- footer-area-end -->