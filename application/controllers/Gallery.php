...
class Gallery extends CI_Controller
{
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->gallery_management->item_count($search);
		$config['per_page'] = "9";
		...
		$data['users_data'] = $this->gallery_management->item_get($config["per_page"], $offset, $search);

		// Generate the pagination links
		$data['pagination'] = $this->pagination->create_links();

		$data['recent'] = $this->gallery_management->gallery_recent();
		// $data['gallery'] = $this->gallery_management->get_gallery();
		$data['content'] = 'webview/gallery/gallery_v';
	...
	public function detail($id = null)
	{
		...
		if ($id) {
			$this->gallery_management->update_count($id);
			$detail = $this->gallery_management->get_id_edit($id);

			if ($detail) {
				$data['detail'] = $detail;
				$data['content'] = 'webview/gallery/gallery_detail_v';
			} else {
				redirect('error404');
			}
		} else {
		...
		// $data['detail'] = $this->gallery_management->get_id_edit($id);
		// $data['content_js'] = 'webview/home/home_js';
		$this->load->view('parts/index_1/wrapper', $data);
	}
}
